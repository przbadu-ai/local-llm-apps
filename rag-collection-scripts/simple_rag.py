...
# Add the parent directory (the project root) to the import search path
...
class SimpleRAG:
    ...
    def __init__(self, path, chunk_size=1000, chunk_overlap=200, n_retrieved=2):
        ...
        Args:
            ...
            n_retrived (int): Number of chunks to retrieve for each query (default: 2).
        """
        print("\n--- Initializing Simple RAG Retriever ---")

        # Encode the PDF document into a vector store using Ollama embeddings
...
# Main function to handle argument parsing and call the SimpleRag class
def main(args):
